...
func HasResponseError(res *http.Response) SdkError {
	...
	if err != nil {
		return Wrap(err, "cannot read error response body")
	}
	newErr.RawBody = body

	err = json.Unmarshal(body, newErr)
	if err != nil {
		return Wrap(err, "could not parse error response body")
	}
...
func unmarshalStandardError(errorType string, body []byte) SdkError {
	...
	if err != nil {
		return Wrap(err, "could not parse error %s response body", errorType)
	}
...
type NoClientCredentialProvidedError struct {
}

// IsScwSdkError implements the SdkError interface
func (e *NoClientCredentialProvidedError) IsScwSdkError() {}
func (e *NoClientCredentialProvidedError) Error() string {
	return fmt.Sprintf("scaleway-sdk-go: no credential option provided")
}
